...
symbols = {
    "SKWH": {
        # more computer function-y symbolse
...
def lookup(chord: list[str]):
    ...
    if any(k in stroke for k in "1234506789"):  # if chord contains a number
        ...
        stroke = ["#"] + stroke
        stroke = "".join(stroke)
        # To avoid 24 14EU9, because that's just stupid
    ...
    # pattern                                                  FRPBLG
    # repetitions                                                         TSDZ
    match = re.fullmatch(r'([#STKPWHR]*)([AO]*)([*-]?)([EU]*)([FRPBLG]*)([TSDZ]*)', stroke)
    ...
    if 'T' in repetitions:
        repeat = repeat + 2
    if 'D' in repetitions or 'Z' in repetitions:
        repeat = repeat + 4
